...
import { replayEvent as ev } from '../data/mock.js';
import { LEVEL_COLOR, levelOf } from '../lib/risk.js';

const KF = ev.keyframes;
const END = KF[KF.length - 1].t;
const KIND_COL = { safe: '#3fd08a', medium: '#f2b53a', high: '#ff5a52' };

function at(t) {
  let i = KF.findIndex((k) => k.t > t);
  if (i === -1) return { ...KF[KF.length - 1], idx: KF.length - 1 };
  if (i === 0) return { ...KF[0], idx: 0 };
  const a = KF[i - 1], b = KF[i], k = (t - a.t) / (b.t - a.t);
  const l = (x, y) => x + (y - x) * k;
  return { ...a, dist: l(a.dist, b.dist), speed: l(a.speed, b.speed), slope: l(a.slope, b.slope), risk: l(a.risk, b.risk), wx: l(a.wx, b.wx), wy: l(a.wy, b.wy), idx: i - 1 };
}
const clockAt = (t) => { const s = 5 + Math.floor(t); return `14:32:${String(s).padStart(2, '0')}`; };

export default function SafetyReplay({ nav }) {
  ...
  const f = at(t);
  const lvl = levelOf(f.risk);
  const alerted = t >= 9;

  return (
    <div>
      <ScreenTitle eyebrow={`${ev.id} · ${ev.date} · ${ev.machine}`} title="Safety replay" sub={ev.summary}
        right={<button className="btn btn-assist" onClick={() => nav('whatif')}><GitCompareArrows size={18} />What if I had…?</button>} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
        <Panel className="xl:col-span-8 !p-0 overflow-hidden">
          <div className="p-5 pb-0 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 -mt-4">
              <span className="num text-2xl">{clockAt(t)}</span>
              <StatusPill level={KF[f.idx].kind === 'safe' ? 'safe' : KF[f.idx].kind}>{KF[f.idx].label}</StatusPill>
            </div>
          </div>
          <div className="relative tech-grid aspect-[16/10] max-w-full">
            <MachinePlan worker={{ x: Math.cos(Math.atan2(f.wy, f.wx)) * (f.dist + 1.7), y: Math.sin(Math.atan2(f.wy, f.wx)) * (f.dist + 1.7) }} distance={f.dist} level={lvl} speed={+f.speed.toFixed(1)} load={78}
        ...
        <div className="xl:col-span-4 grid sm:grid-cols-2 xl:grid-cols-1 gap-4 content-start">
          <Panel>
            <div className="flex items-center gap-4">
              <div className="w-[150px] shrink-0"><ArcGauge value={Math.round(f.risk)} level={lvl} size={150} label={lvl.toUpperCase()} /></div>
              <div className="space-y-3 flex-1">
                <Read k="Distance" v={`${f.dist.toFixed(1)} m`} warn={f.dist < 3} />
                <Read k="Speed" v={`${f.speed.toFixed(1)} km/h`} warn={f.speed > 5} />
          ...
          <Panel>
            ...
            <ol className="space-y-2.5 text-sm">
              <li className="flex gap-2"><span className="num text-ink3">1</span><span>Worker entered proximity zone 6 s before the alert.</span></li>
              <li className="flex gap-2"><span className="num text-ink3">2</span><span>Speed rose from 4.4 to 8.0 km/h while distance was decreasing.</span></li>
              <li className="flex gap-2"><span className="num text-ink3">3</span><span>Operator stopped within 3 s of the alert — <span className="text-safe">good response</span>.</span></li>
            </ol>
          </Panel>
        </div>
        ...
        <Panel className="xl:col-span-12">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <span className="num text-sm text-ink2">{t.toFixed(1)} s / {END}.0 s</span>
          </div>
          <Timeline t={t} onSeek={(x) => { setT(x); setPlaying(false); }} />
        </Panel>
      </div>
    </div>
  );
...
function Timeline({ t, onSeek }) {
  const W = 1000, H = 90;
  const x = (s) => 20 + (s / END) * (W - 40);
  const y = (r) => H - 8 - (r / 100) * (H - 20);
  const pts = Array.from({ length: 121 }, (_, i) => (i / 120) * END).map((s) => [x(s), y(at(s).risk)]);
    ...
    <div className="overflow-x-auto">
      <div className="min-w-[640px]">
        <svg ref={ref} viewBox={`0 0 ${W} ${H}`} width="100%" height={H} preserveAspectRatio="none" onClick={seek} className="cursor-pointer" aria-label="Risk over the event">
          ...
          {[35, 62].map((th) => <line key={th} x1="20" x2={W - 20} y1={y(th)} y2={y(th)} stroke={th === 35 ? '#f2b53a' : '#ff5a52'} strokeOpacity=".35" strokeDasharray="4 5" vectorEffect="non-scaling-stroke" />)}
